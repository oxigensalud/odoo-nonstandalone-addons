...
class TestSpmsVerification(SavepointCase):
    ...
    def test_without_errors_result_closes_as_zero(self):
        move = self._standard_invoice()
        result = self._create_result(move, [], verification_state="without_errors")
        self.assertEqual(result.state, "zero_official")
    ...
    def test_generate_negative_claim_gets_own_negative_line(self):
        # the verification allowed TESTP021 above the billed amount: its negative
    ...
    def test_generate_official_exceeds_total_blocks(self):
        ...
        # verification-data anomaly: the guard fires before the draft
        move = self._standard_invoice()
    ...
    def test_generate_negative_official_mixed_claims(self):
        # an over-billed claim and a larger one the verification allowed above the
    ...
    def test_foreign_note_of_the_other_kind_does_not_block(self):
        ...
        # verification, and a credit note never blocks a debit result
        move = self._standard_invoice()
    ...
    def test_error_under_line_of_another_verification_blocks(self):
    ...
    def test_error_source_cannot_be_reassigned(self):
        ...
        self.assertEqual(error.result_id, result)
        self.assertEqual(error.line_id, result.line_ids)
        # Moving within the same verification also loses the original claim.
        other_line = self.env["spms.invoice.verification.line"].create(
    ...
    def test_document_error_source_cannot_be_reassigned(self):
        ...
        error = (
            self.env["spms.invoice.verification.error"]
            .create(
                {
                    "result_id": result.id,
                    "level": "invoice",
    ...
    def test_document_error_with_line_blocks(self):
        ...
        with self.assertRaises(ValidationError):
            self.env["spms.invoice.verification.error"].create(
                {
                    "result_id": result.id,
    ...
    def test_error_list_defaults_to_the_errors_that_carry_money(self):
        ...
        errors of the claims the verification cut or raised plus the errors
        ...
        document_error = self.env["spms.invoice.verification.error"].create(
            {
                "result_id": result.id,
                "level": "invoice",
                "error_type_id": self.env["spms.invoice.verification.error.type"]
                ._get_or_create("C313")
                .id,
        ...
        menu = self.env.ref(
            "l10n_pt_invoice_spms_verification.spms_invoice_verification_error_menu"
        )
        action = self.env.ref(
            "l10n_pt_invoice_spms_verification.spms_invoice_verification_error_action"
        )
        ...
        errors = self.env["spms.invoice.verification.error"]
