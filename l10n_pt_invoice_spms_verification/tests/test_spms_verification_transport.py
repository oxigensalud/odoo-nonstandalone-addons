...
MODULE = "odoo.addons.l10n_pt_invoice_spms_verification.models.edi_exchange_record"
CLIENT_PATH = MODULE + ".EdiExchangeRecord._l10n_pt_spms_verification_client"
FETCH_PATH = MODULE + ".EdiExchangeRecord._l10n_pt_spms_verification_fetch"
TRANSPORT_PATH = MODULE + ".SpmsVerificationTransport"
...
DOCUMENT_XML = (
    ...
    "<ApplicationResponse><SyntheticVerificationResult/></ApplicationResponse>"
)
...
NOT_VERIFIED_YET = "302 - Factura ainda não conferida."


def _result_response(inner):
    """A verification answer as the live service sends it: HTTP 200 with the
...
def _empty_response():
    """A verification answer without the <return> element, which the WSDL
    allows (minOccurs=0)."""
    return (
        200,
        (
...
class _FakeTransport(SpmsVerificationTransport):
...
class TestSpmsVerificationTransport(SavepointComponentCase):
    ...
    def _results(self):
        return self.env["spms.invoice.verification"].search(
            [("move_id", "=", self.invoice.id)]
        )

    def _settle(self):
        """A definitive verification result on the invoice, out of band."""
        return self.env["spms.invoice.verification"].create(
            {"move_id": self.invoice.id, "verification_state": "without_errors"}
        )
    ...
    def test_service_illness_is_an_error_on_reception(self):
        ...
        # next pass — the job itself stays green; the next "not verified
        # yet" clears the error by itself
        transport = _FakeTransport(
            _fault_response("999 - Erro desconhecido."),
            _fault_response(NOT_VERIFIED_YET),
        )
        self._run_cron(transport)
        child = self._children()
    ...
    def test_echoed_request_is_not_persisted(self):
        ...
        self.assertIn(
            "_l10n_pt_spms_verification_fetch", child.exchange_error_traceback
        )
        self.assertFalse(self._results())
    ...
    def test_answer_without_document_is_an_error_on_reception(self):
        ...
        for answer in (
            ...
            with self.subTest(answer=answer):
                ...
                child = self._children()
                self.assertEqual(child.edi_exchange_state, "input_receive_error")
                self.assertIn("without a verification document", child.exchange_error)
                self.assertFalse(child.exchange_error_traceback)
                self.assertFalse(self._results())
    ...
    def test_failed_poll_of_settled_invoice_left_alone(self):
        # the failed job of an invoice verified meanwhile is not revived
        record = self._fail_queued_poll()
        self._settle()
        self.assertEqual(self._queued_polls(), 0)
        self.assertEqual(record.state, "failed")
    ...
    def test_job_skips_invoice_settled_meanwhile(self):
        ...
        self._settle()
        with mock.patch(CLIENT_PATH) as factory:
            child.action_l10n_pt_spms_verification_poll()
        factory.assert_not_called()

    def test_verification_result_is_the_ack_of_the_sent_invoice(self):
        # edi_oca models an expected answer document as the ACK of the
        # exchange: the sending type of l10n_pt_invoice_spms names our
        # verification result type as its ACK type, so a sent invoice knows it
        # is waiting for its result and shows when it arrived
        send_type = self.env.ref("l10n_pt_invoice_spms.spms_exchange_type")
        verification_type = self.env.ref(
            "l10n_pt_invoice_spms_verification.spms_verification_exchange_type"
        )
        self.assertEqual(send_type.ack_type_id, verification_type)
        self.assertTrue(self.exchange.ack_expected)
    ...
    def test_credit_note_not_polled(self):
        ...
        the CCF verifies invoices only: a nota is never polled, its sent
        record expects no verification result and gets no result record."""
        self._settle()
    ...
    def test_cron_ships_active(self):
        ...
        cron = self.env.ref(
            "l10n_pt_invoice_spms_verification.cron_spms_verification_update"
        )
        self.assertTrue(cron.active)
        self.assertIn("_cron_l10n_pt_spms_verification_update", cron.code)
