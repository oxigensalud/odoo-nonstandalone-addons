...
class SpmsInvoiceVerificationError(models.Model):
    """One row per error (Erro) reported by the verification document.
    ...
    _description = "SPMS Invoice Verification Error"
    ...
    error_type_id = fields.Many2one(
        ...
        index=True,
        ondelete="restrict",
        help="Type of this error in the catalogue of verification error types; "
    ...
    code = fields.Char(
        ...
        index=True,
        string="Error Code",
        help="Error code reported by the verification (Erro/Codigo), "
        "e.g. C010, C012, C313.",
    )
    ...
    move_id = fields.Many2one(
        ...
        store=True,
        readonly=True,
        help="Invoice the verification reported this error on.",
    )
    ...
    def _check_anchor(self):
        """An error hangs under a line of its own verification, and only the
    ...
    def write(self, vals):
        ...
        # reassigns an existing error to a different verification or claim.
        for rec in self:
            if any(
