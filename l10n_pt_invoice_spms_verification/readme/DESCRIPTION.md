...
- Stores one verification result per invoice — the CCF document number and
  ...
  the verification document states it) and the official result notice, the
  CCF's ofício.
- Stores the verification breakdown the way the document states it: one line
  per claim (prescription) carrying the read and recomputed claim totals
  and their difference, and under it every error the verification reports with
...
- Generates the draft rectifying document for the results that came back
  ...
  value is negative (the verification computed more than billed) — carrying
