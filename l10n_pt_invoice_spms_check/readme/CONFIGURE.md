...
- The polling scheduled action (*SPMS: obtain check results*, under
  *Settings → Technical → Automation → Scheduled Actions*) runs hourly out
  of the box. It only acts on invoices already sent to SPMS that still
  have no definitive check result, with each company's own SPMS
