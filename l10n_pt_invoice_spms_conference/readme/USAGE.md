Every invoice sent to SPMS gets its conference result attached the moment
the conference resolves it: the invoice form shows a smart button with the
error count, opening the result directly (one result per invoice — the
first definitive answer closes the invoice permanently).

The result form carries the conference state, the official totals and the
...
For a result conferred with errors, a draft rectifying invoice is created
