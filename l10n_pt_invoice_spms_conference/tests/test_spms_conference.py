...
class TestSpmsConference(SavepointCase):
    """Exercise the conference-result store and the credit-note generation.

    The web-service transport and the conference-document parser are not
    ...
    @classmethod
    def _create_result(
        cls, move, rows, credit_official=0.0, conference_state="with_errors"
    ):
        """Build a conference result the way the parser will.
        ...
        """
        result = cls.env["spms.conference.result"].create(
            {
                "move_id": move.id,
        ...
        cls.env["spms.conference.result.error"].create(error_vals_list)
        return result
    ...
    def test_without_errors_result_closes_as_zero(self):
        move = self._standard_invoice()
        result = self._create_result(move, [], conference_state="without_errors")
        self.assertEqual(result.state, "zero_official")
    ...
    def test_generate_official_exceeds_total_blocks(self):
        ...
        # conference-data anomaly: the guard fires before the draft, even
